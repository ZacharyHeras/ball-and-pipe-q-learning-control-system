...
str = read(device, 20, "string");
%% Translate
% translate the response to 4 doubles using str2double() and
% extractBetween() (Hint: the response is in the spec sheet)
distance   = str2double(extractBetween(str, 2, 5));
pwm        = str2double(extractBetween(str, 7, 10));
target     = str2double(extractBetween(str, 12, 15));
deadpan    = str2double(extractBetween(str, 17, 20));
end